#!/usr/bin/env bash
###############################################################################
# DTU Sustain – openSUSE Tumbleweed – Module: Software Installation
###############################################################################
set -euo pipefail
...
if (( ${#SNAP_APPS[@]} > 0 )); then
    ...
    if ! command -v snap &>/dev/null; then
        ...
        # snapd needs a moment after first enable
        sleep 5
    fi
...
if $CISCO_ENABLED; then
    ...
    if [[ -z "$CISCO_TAR" || ! -f "$CISCO_TAR" ]]; then
        ...
        zypper --non-interactive install -y libxml2-2 kernel-devel gcc make 2>/dev/null || true
