...
# Mirrors the proven mount-mdrive-kubuntu.sh logic (test-mount discovery,
# vers=3.0/ntlmssp/nodfs options, systemd automount with nofail).
# ─────────────────────────────────────────────────────────────────────────────
...
DCONF
  ...
  ls "$M_MOUNTPOINT" >/dev/null 2>&1 && ok "M-Drive reachable at ${M_MOUNTPOINT}" \
    || warn "M-Drive not mounted on first access yet. Try: ls ${M_MOUNTPOINT}"
  if [[ -n "$O_SHARE" ]]; then
    ls "$O_MOUNTPOINT" >/dev/null 2>&1 && ok "O-Drive reachable at ${O_MOUNTPOINT}" \
      || warn "O-Drive not mounted on first access yet. Try: ls ${O_MOUNTPOINT}"
  fi
...
echo "[5b/8] Scanning ${M_USERS_BASE} for M-Drive on //${M_SERVER} (test-mount)..."
...
if M_SUBDIR="$(cifs_find_mdrive_subdir "$M_SERVER" "$M_USERS_BASE" "$USERNAME" "$CREDS_FILE" "$UID_NUM" "$GID_NUM" "$M_CACHE")"; then
  ...
  echo "[5c/8] Writing M-Drive fstab entry..."
