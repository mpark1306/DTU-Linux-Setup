...
#       The share \\ait-pdfs\Qdrev\SUS is a DFS junction pointing to:
#         \\ait-pqumulo.win.dtu.dk\sus-q$
...
module_qdrive() {
  ...
  DOMAIN="WIN"
  # Real DFS target (\\ait-pdfs\Qdrev\SUS → \\ait-pqumulo.win.dtu.dk\sus-q$)
  SERVER="ait-pqumulo.win.dtu.dk"
  SHARE="sus-q\$"
  MOUNTPOINT="/mnt/Qdrev"
  CREDS_FILE="/home/$USERNAME/.smbcred-ait-pdfs"
...
EOF
  ...
  sed -i "/ait-pdfs.*[Qq]drev/d" "$FSTAB_FILE" 2>/dev/null || true
  ...
  if mount | grep -qE "[[:space:]]${MOUNTPOINT}[[:space:]]"; then
    ...
    echo "    (DFS junction from \\\\ait-pdfs\\Qdrev\\SUS)"
...
module_defender() {
  ...
  curl -fsSL -o /tmp/MicrosoftDefenderATPOnboardingLinuxServer.py \
    https://defender.ait.dtu.dk/download/MicrosoftDefenderATPOnboardingLinuxServer.py
